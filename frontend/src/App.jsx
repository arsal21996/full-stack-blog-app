...
function App() {
  ...
  useEffect(() => { const timer = setTimeout(() => loadRecipes(), 280); return () => clearTimeout(timer); }, [query]);
  ...
  async function toggleFavorite(recipe) {
    ...
    } catch { setError('Favorites need the HomeCooked API to be running.'); }
  }
  ...
  return (
    <div className="site-shell">
      <header className="nav-wrap"><nav className="nav container">
        <button className="brand" onClick={() => { setSelected(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }}><span className="brand-mark">HC</span><span>home<span>cooked</span></span></button>
        <div className={`nav-links ${menuOpen ? 'open' : ''}`}><a href="#recipes" onClick={() => setMenuOpen(false)}>Recipes</a><a href="#categories" onClick={() => setMenuOpen(false)}>Collections</a><a href="#about" onClick={() => setMenuOpen(false)}>Our kitchen</a></div>
      ...
      <main>
        <section className="hero container"><div className="hero-copy"><p className="eyebrow"><Sparkles size={15} /> Recipes worth gathering around</p><h1>Good food.<br /><em>Good mood.</em></h1><p className="hero-text">Simple, generous recipes for slow Sundays, busy weeknights and everything delicious in between.</p><div className="search-box"><Search size={20} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search recipes, ingredients..." /><kbd>⌘ K</kbd></div></div>
          {featured && <button className="hero-card" onClick={() => setSelected(featured)}><img src={featured.image} alt={featured.title} /><div className="hero-card-copy"><span>Editor's pick</span><strong>{featured.title}</strong><small><Clock3 size={14} /> {featured.time}</small></div><span className="circle-arrow"><ArrowRight size={18} /></span></button>}
        </section>
        <section className="category-strip container" id="categories"><div className="section-kicker"><span>Browse by craving</span><ChevronRight size={16} /></div><div className="category-row">{categories.map((item) => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div></section>
        <section className="recipes-section container" id="recipes"><div className="section-heading"><div><p className="eyebrow">From our kitchen</p><h2>Make something <em>lovely.</em></h2></div><span className="recipe-count">{recipes.length} recipes</span></div>{error && <div className="notice">{error}</div>}{loading ? <div className="loading-grid">{[1,2,3].map((x) => <div className="skeleton" key={x} />)}</div> : recipes.length === 0 ? <div className="empty">No recipes found. Try another ingredient or category.</div> : <div className="recipe-grid">{recipes.map((recipe, index) => <RecipeCard key={recipe.id} recipe={recipe} index={index} saved={saved.includes(recipe.id)} onSave={() => toggleSave(recipe.id)} onFavorite={() => toggleFavorite(recipe)} onOpen={() => setSelected(recipe)} />)}</div>}</section>
        <section className="quote-section" id="about"><div className="container quote-inner"><Star size={18} fill="currentColor" /><blockquote>“The best recipes don't just feed people. They give everyone a reason to stay at the table a little longer.”</blockquote><p>— The HomeCooked kitchen</p></div></section>
      </main>

...
app.get('/api/health', (_req, res) => {
  res.json({ status: 'ok', service: 'HomeCooked API' });
});

app.get('/api/categories', async (_req, res, next) => {
  try {
    const recipes = await readRecipes();
    res.json([...new Set(recipes.map((recipe) => recipe.category))]);
  } catch (error) {
    next(error);
  }
});
...
app.get('/api/recipes', async (req, res, next) => {
  try {
    ...
    const filtered = recipes.filter((recipe) => {
      const matchesSearch = !search || [recipe.title, recipe.description, recipe.category, ...recipe.ingredients]
...
app.post('/api/recipes', async (req, res, next) => {
  try {
    ...
    if (!title || !category || !description || !ingredients?.length || !instructions?.length) {
      return res.status(400).json({ message: 'Title, category, description, ingredients and instructions are required.' });
    }
    const recipes = await readRecipes();
    const recipe = {
      id: crypto.randomUUID(), title, category, description,
      image: image || 'https://images.unsplash.com/photo-1498837167922-ddd27525d352?auto=format&fit=crop&w=1200&q=85',
      time: time || '30 min', difficulty: difficulty || 'Easy', servings: Number(servings) || 2,
      author: author || 'HomeCooked Community', ingredients, instructions
    };
